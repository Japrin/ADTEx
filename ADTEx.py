...
def sortFile(inF,fileN):
	...
	subprocess.call("gzip -cd %s | sort -V -k1 -k2n -k3n -k4n > %s" %(inFile,inFile+".sorted"),shell=True)
...
def analyseCNV(params,ratio_data,outF,chroms):
	...
	def runCNV(par,ratios,outLoc,chrom,rScr,rFunctions,p):
		print ("Rscript %s %s %s %s %s %s %s %s %s %s" %(rScr,rFunctions, ratios,p,par.minRead,outLoc,par.bafin,par.baf,par.plot,chrom))
		args = shlex.split("Rscript %s %s %s %s %s %s %s %s %s %s" %(rScr,rFunctions, ratios,p,par.minRead,outLoc,par.bafin,par.baf,par.plot,chrom))
...
def zygosity(params,outF,chroms):
	...
	print ("Rscript %s %s %s %s %s %s" %(rScriptName,params.baf,outF,params.minRead,params.plot,chroms))
...
def main():
	...
	try:
		...
		print "out folder already exists '%s'" %outF
	...
	try:
		...
		print "temp folder already exists:\n\t%s\n\t%s\n\t%s\n" % (outF+"/temp",outF+"/temp/control",outF+"/temp/tumor")
	...
	if (str(docInput)=="True"):
		...
		subprocess.call("ln -s %s %s" %(control,outF+"/temp/control/coverage.txt"),shell=True)
		subprocess.call("ln -s %s %s" %(tumor,outF+"/temp/tumor/coverage.txt"),shell=True)
