...
for (i in chrom){
  ...
  #filter<-control[,1]==i & control[,4]>thresh
  #tumour<-tumor[filter,5]
  #normal<-control[filter,5]
  #exome<-control[filter,1:3]
  #colnames(exome)<-c("Chr","Start","End")
  #data<-generate.data(control=normal,tumor=tumour,exome=exome)
  f<-result$chr==i
  if(i==chrom[1]){
...
if(bafin){
  ...
  for(i in chrom){
    ...
    if(length(changes)>10 & length(table(path))<4 & sum(path!=ploidy)>100){
      ### only chrom with more than 100 het snp; 
      ### segment the het snp ratios
      if(sum(baf$chrom==i)>100){
...
#baf$cnv<-rep(NA,length(baf$chrom))
#for (i in chrom){
  #d1<-baf[baf$chrom==i,]
  #d2<-result[result$chr==i,]
  #for(j in 1:nrow(d2)){
    #k<-which(d2$CNV_start[j]<=d1$SNP_loc & d2$CNV_end[j]>=d1$SNP_loc)
    #baf$cnv[baf$chrom==i][k]<-d2$cnv
  #}
#}
