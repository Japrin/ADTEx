  ...
  for(model in models){
    ...
    if(kk$BIC<BIC){#selecting best model with minimum BIC (i.e. min. information loss)
      if(all(diff(kk$mu)>=0)){ 
        ...
        #if(any(diff(kk$mu)<0.05) | kk$mu[normal.state+1] <(mean+0.1) | kk$mu[normal.state-1] >(mean-0.1)){
         # if(length(predict)>0){
          #  fitted.model$mu<-predict[["mu"]]
           # fitted.model$model<-predict[["model"]]
          #  id<-which(fitted.model$model==d)
          #  fitted.model$sigma<-fitted.model$sigma[(id-1):(id+1)]
          #  fitted.model$A<-A[(id-1):(id+1),(id-1):(id+1)]
          #  fitted.model$pi<-pi[(id-1):(id+1)]
          #  nstates<-length(fitted.model$model)
          #  fitted.model$m<-nstates
          #}
          #else{
          #  fitted.model$mu<-mu
          #}
